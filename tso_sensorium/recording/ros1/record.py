"""ROS 1 topic recording adapters.

Subscribes to ROS topics and delegates file writing to the framework-free
recorders in ``tso_sensorium.recording.core``. A ``Recorder`` orchestrates
multiple topic recorders and an optional rosbag capture.
"""
...
ROSBAG_FILE_NAME = "recording.bag"


class RosTopicRecorder:
    """Records a ROS topic to a timestamped CSV file.

    Messages are timestamped with their header stamp when available,
    falling back to the receive time.
...
class Recorder:
    """Orchestrates multiple topic recorders and optional rosbag capture.

    Args:
        output_folder: Base directory for all recordings.
        record_rosbag: Whether to also record a rosbag of the registered
            rosbag topics.
    """

    def __init__(self, output_folder: Path | str, record_rosbag: bool = False):
    ...
    def add_rosbag_topics(self, topics: list[str]) -> "Recorder":
        """Set the topics captured by the rosbag recording.
    ...
    def start_recording(self) -> None:
        ...
        if self.record_rosbag and self.rosbag_topics:
            rosbag_file = str(Path(self.output_folder, ROSBAG_FILE_NAME))
            command = ["rosbag", "record", "-O", rosbag_file]
            self.rosbag_process = subprocess.Popen(command + self.rosbag_topics)
            rospy.loginfo("Started rosbag recording: %s", rosbag_file)
        for recorder in self.recorders:
            recorder.subscribe()

    def close(self) -> None:
        """Close all recorders and stop the rosbag capture."""
        for recorder in self.recorders:
            recorder.close()
